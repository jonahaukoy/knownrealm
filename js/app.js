...
(function () {
  ...
  const state = {
    ...
    lore: localStorage.getItem("kw-lore") || "show", // which telling: "show" | "book"
  ...
  const SEAT_CONTROL = {
    // Greyjoy flag corrected to BOOK ch 38 (Theon III, the chapter Theon takes
    // Winterfell) — it used to fly a chapter early at ch 37 (Tyrion VIII). Show
    // Bolton moved to S3E9 (the Red Wedding) to match the North; book Bolton to
    // mid-ADWD (ch 34), when Roose's host reoccupies the rebuilt Winterfell.
    "winterfell":    { start: "stark",     show: [[2,6,"greyjoy"],[2,10,null],[3,9,"bolton"],[6,9,"stark"]],
                       book: [[2,38,"greyjoy"],[2,67,null],[5,34,"bolton"]] },
    // The Blackfish retakes Riverrun for two episodes (S6E7–E8) before it falls
    // back to the Freys at S6E9; the Frey takeover is the Red Wedding, S3E9 (not
    // E10). In the books the Tullys hold Riverrun through the siege until Jaime
    // ends it in AFFC ch 44 (Jaime V) — previously mis-set to ch 33 (Brienne VI).
    "riverrun":      { start: "tully",     show: [[3,9,"frey"],[6,7,"tully"],[6,9,"frey"],[8,6,"tully"]],
                       book: [[4,44,"frey"]] },
    // Harrenhal passes from lord to lord: House Whent's arms (its minor shield)
    // until it is occupied — Tywin's Lannister host, then Roose Bolton, then
    // emptied. Show: Tywin S2E4, Bolton S3E2, abandoned S4E1. Book: Lannister
    // ch 20 (Arya brought there), Bolton ch 31 (Arya VII, "changes hands to
    // Roose Bolton"), emptied ch 50 when Bolton marches for the Red Wedding.
    "harrenhal":     { start: null,        show: [[2,4,"lannister"],[3,2,"bolton"],[4,1,null]],
                       book: [[2,20,"lannister"],[2,31,"bolton"],[3,50,null]] },
    "dragonstone":   { start: "baratheon", show: [[6,1,null],[7,1,"targaryen"]],
                       book: [[4,40,"lannister"]] },
    "highgarden":    { start: "tyrell",    show: [[7,3,"lannister"]], book: [] },
    "casterly-rock": { start: "lannister", show: [[7,3,"targaryen"],[7,5,"lannister"]], book: [] },
    "kings-landing": { start: "baratheon", show: [[6,10,"lannister"],[8,5,"targaryen"],[8,6,"stark"]],
                       book: [] },
    // the stag flies for Renly, then Stannis; comes down when Stannis abandons
    // the south for the Wall (show), and rises again for Gendry. In the books
    // the stag never comes down: Ser Gilbert Farring holds Storm's End for
    // Stannis to the last page (the Golden Company has not taken it by ADWD's
    // end), so the book timeline stays Baratheon.
    "storms-end":    { start: "baratheon", show: [[5,1,null],[8,4,"baratheon"]],
                       book: [] },
    // the dragon queen's city: her banner rises when she takes Meereen and
    // stays over the city she rules (show S4E3; books ASOS Daenerys VI)
    "meereen":       { start: null, show: [[4,3,"targaryen"]], book: [[3,43,"targaryen"]] },
    // wiped-out houses take their banners down with them
    // (minor: true = only suppresses the seat's small shield, never adds one)
    "the-twins":     { start: "frey",   show: [[7,1,null]], book: [] },              // Arya's toast
    "the-dreadfort": { start: "bolton", show: [[6,9,null]], book: [], minor: true }, // the last Bolton fed to his hounds
    "last-hearth":   { start: "umber",  show: [[8,1,null]], book: [], minor: true }, // little Ned Umber, and the dead after
  };
  ...
  function bannerSeats() {
    const seats = [];
    const greatShieldSeats = new Set(); // seats now flying a great holder's shield
    ...
    // controlled seats that aren't a great house's home (the capital, a castle
    // that changes hands like Harrenhal) fly the current holder's shield once
    ...
    WORLD.locations.forEach((loc) => {
      ...
      // a great holder's shield flies here now (e.g. Harrenhal under Lannister
      // or Bolton) — so the castle's own minor arms give way to it
      if (greatShieldSeats.has(loc.id)) return;
      // a wiped-out minor house takes its shield down (SEAT_CONTROL minor: true
      // entries resolving to null — the last Bolton, the last Umber)
      const sc = SEAT_CONTROL[loc.id];
      if (sc && sc.minor && storyPoint() && seatHouseAt(loc.id, "keep") === null) return;
  ...
  function setSite(site) {
    ...
    els.siteTitle.textContent = isWordle ? "WORDLE OF THE REALM" : "GAME OF THRONES MAP";
    document.title = isWordle
      ? "Wordle of the Realm - Game of Thrones"
      : "Game of Thrones Map - A Chronicle of the Known World";
  ...
  function initSiteSwitcher() {
    ...
    els.siteDropdown.querySelectorAll("[data-site]").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        else if (btn.dataset.site === "hotd") window.location.href = "hotd/index.html";
  ...
  const WORDLE_POOL = {
    names: ["AEGON", "AEMON", "AERON", "AERYS", "BALON", "BERIC", "BRONN", "DAVOS", "DORAN", "DROGO", "EURON", "GILLY", "GRENN", "HODOR", "JAIME", "JAQEN", "JOJEN", "JORAH", "KEVAN", "LORAS", "LUWIN", "MANCE", "MEERA", "OBARA", "PETYR", "RENLY", "ROBIN", "ROOSE", "SANSA", "SELMY", "SYRIO", "THEON", "TYENE", "TYWIN", "VARYS", "YOREN"],
    places: ["DORNE", "ESSOS", "EYRIE", "NORTH", "QARTH", "QOHOR", "REACH", "TWINS"],
    houses: ["ARRYN", "DAYNE", "REYNE", "ROYCE", "STARK", "SWANN", "TARLY", "TARTH", "TULLY", "UMBER"],
  ...
  function buildMenus() {
    byId("menu-map").innerHTML = `
      ...
      <label class="legend-row"><input type="checkbox" id="f-people" ${state.filters.people ? "checked" : ""}/>
        <span class="legend-face-icon"><img src="assets/people/tyrion-lannister.jpg" alt=""/></span> People of the Story <span class="legend-hint">(when an episode or chapter is chosen)</span></label>
  ...
  function renderWelcome() {
    els.sidebar.innerHTML = `
      <div class="sidebar-title">A Chronicle of Ice and Fire</div>
      <p class="sidebar-note">This is the known world as the tale opens, in the two hundred and ninety-eighth year after Aegon's Conquest — the year the Hand died, the wolves went south, and dragons woke in the east. From here the map follows the whole story, telling by telling.</p>
      <div class="sidebar-divider"></div>
      ...
      <div class="sidebar-divider"></div>
      <p class="sidebar-note">Drag to pan and scroll to zoom. Click any marker to read its chronicle here. Hover the ribbons above — <i>The Map, People &amp; Houses, Regions</i> — to choose what the map shows. Pick a season &amp; episode, or a book &amp; chapter, from <i>The Story</i> (top right) and the whole map follows.</p>
      <div class="sidebar-divider"></div>
  ...
  function buildSeasonDropdown(expanded) {
    ...
    let s = `<div class="story-tabs">
      ...
      <button class="story-tab ${storyShelf === "books" ? "active" : ""}" data-shelf="books">&#128214; The Books</button>
    </div>
    ...
    if (storyShelf === "show") {
      ...
      s += `<div class="dropdown-title">The Song of Ice and Fire, Book by Book</div>`;
      BOOKS.forEach((book) => {
  ...
  function showEpisodePlayer(seasonN, n) {
    ...
    if (state.scrubAll) {
      ...
      els.playerJourneySummary.textContent = "the eight seasons on one line";
      els.scrubber.min = "1";
  ...
  function setLore(lore) {
    state.lore = lore;
    localStorage.setItem("kw-lore", lore);
