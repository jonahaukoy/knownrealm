/* HOUSE OF THE DRAGON — an ALLEGIANCE map of the Dance of the Dragons.
   Same engine as the ASOIAF map: polygons drawn generously past the coast and
   clipped to dry land by the landmask; `base` ruler + optional `own` (show,
   [season,episode,ruler]) and `ownBook` (Fire & Blood, [1,chapter,ruler])
   timelines; null dissolves; `sub: true` overlays; `nomask: true` skips the
   landmask (small isles). The realm opens WHOLE under Viserys's gold — then
   splits into the queen's BLACKS and the king's GREENS, while Dorne watches
   from behind its mountains and the krakens weigh both crowns. The book's
   war runs to its true end: the boy king's gold reunites the map. */

const RULERS = {
  "iron-throne": { name: "The Iron Throne — the realm at peace",       color: "#e0a410" },
  "blacks":      { name: "Queen Rhaenyra — the Blacks",                color: "#2b2026" },
  "greens":      { name: "King Aegon II — the Greens",                 color: "#3f6b3a" },
  "greyjoy":     { name: "House Greyjoy — krakens between crowns",     color: "#7a3fb0" },
  "martell":     { name: "Dorne — unbowed, and no part of this",       color: "#e06414" },
  "nights-watch": { name: "The Night's Watch",                         color: "#3b4046" },
  "free-folk":   { name: "The Free Folk",                              color: "#77876f" },
  "masters":     { name: "The Masters",                                color: "#8a6d3b" },
};

const TERRITORIES = [

  /* ---------------- beyond the Wall: no Dance up here ---------------- */
  { id: "bw-far", name: "The Lands of Always Winter", base: "free-folk",
    pts: [[240,40],[1660,40],[1660,120],[1200,125],[700,135],[400,140],[255,100]] },
  { id: "bw-mid", name: "The haunted forest, deep", base: "free-folk",
    pts: [[255,120],[400,175],[700,165],[1200,150],[1660,140],[1660,192],[1100,208],[600,232],[300,225]] },
  { id: "bw-near", name: "The haunted forest", base: "free-folk",
    pts: [[300,225],[600,232],[1100,208],[1660,192],[1660,240],[1620,252],[1546,248],[1424,262],[1300,272],[1180,285],[1100,310],[950,340],[700,350],[400,360],[260,300]] },

  /* ---------------- the North: the Pact of Ice and Fire ---------------- */
  { id: "the-north", name: "The North", base: "iron-throne",
    own: [[2,1,"blacks"]],
    ownBook: [[1,2,"blacks"],[1,8,"iron-throne"]],
    pts: [[360,380],[950,340],[1130,372],[1200,418],[1300,452],[1420,478],[1560,460],[1620,430],[1645,340],[1720,405],[1950,445],[1950,1230],[1600,1255],[1400,1240],[1250,1232],[1100,1258],[900,1285],[830,1310],[720,1352],[600,1362],[500,1345],[420,1330],[360,1335]] },
  { id: "the-north-isles", name: "Skagos & the northern isles", base: "iron-throne",
    own: [[2,1,"blacks"]],
    ownBook: [[1,2,"blacks"],[1,8,"iron-throne"]],
    pts: [[1670,150],[1950,140],[1950,415],[1760,420],[1690,315]] },
  { id: "the-gift", name: "The Gift & the New Gift", base: "nights-watch",
    pts: [[1100,310],[1180,285],[1300,272],[1424,262],[1546,248],[1620,252],[1645,340],[1620,430],[1560,460],[1420,478],[1300,452],[1200,418],[1130,372]] },

  /* ---------------- the Vale: the Maiden of the Vale declares ---------------- */
  { id: "the-vale", name: "The Vale", base: "iron-throne",
    own: [[1,10,"blacks"]],
    ownBook: [[1,2,"blacks"],[1,8,"iron-throne"]],
    pts: [[1462,1330],[1520,1430],[1500,1520],[1478,1620],[1505,1720],[1560,1700],[1750,1680],[1950,1650],[1950,1285],[1830,1278],[1690,1288],[1580,1315],[1500,1345]] },

  /* ---------------- the Riverlands: Harrenhal falls, the rivers go black ---------------- */
  { id: "the-riverlands", name: "The Riverlands", base: "iron-throne",
    own: [[2,3,"blacks"]],
    ownBook: [[1,3,"blacks"],[1,8,"iron-throne"]],
    pts: [[600,1362],[720,1352],[830,1310],[900,1285],[1100,1258],[1250,1232],[1400,1240],[1560,1258],[1462,1330],[1520,1430],[1500,1520],[1478,1620],[1505,1720],[1560,1700],[1560,1810],[1620,1848],[1500,1898],[1440,1925],[1350,1870],[1260,1860],[1230,1920],[1240,2000],[1270,2080],[1170,2075],[1080,2060],[1030,1960],[1010,1880],[980,1820],[900,1750],[870,1655],[920,1640],[940,1500],[930,1370]] },

  /* ---------------- the Iron Islands: the Red Kraken weighs both crowns ---------------- */
  { id: "iron-islands", name: "The Iron Islands", base: "greyjoy",
    pts: [[560,1430],[820,1430],[870,1470],[930,1545],[900,1620],[860,1650],[790,1655],[700,1690],[590,1670],[520,1560]] },

  /* ---------------- the Westerlands: the lions for the green king ---------------- */
  { id: "the-westerlands", name: "The Westerlands", base: "iron-throne",
    own: [[1,10,"greens"]],
    ownBook: [[1,2,"greens"],[1,8,"iron-throne"]],
    pts: [[760,1660],[870,1655],[900,1750],[980,1820],[1010,1880],[1030,1960],[1080,2060],[950,2140],[850,2200],[740,2240],[640,2270],[480,2240],[510,1780],[730,1715]] },

  /* ---------------- the Crownlands (Crackclaw Point merged in, see GoT) ------- */
  { id: "the-crownlands", name: "The Crownlands", base: "iron-throne",
    pts: [[1560,1700],[1750,1680],[1950,1650],[1950,1878],[1725,1878],[1725,1985],[1950,1985],[1950,2225],[1650,2228],[1520,2212],[1400,2190],[1330,2140],[1270,2080],[1240,2000],[1230,1920],[1260,1860],[1350,1870],[1440,1925],[1500,1898],[1620,1848],[1560,1810]] },

  /* The bay's north coast — Duskendale and Rook's Rest — keeps the queen's
     oath until the Kingmaker's host arrives to collect it. */
  { id: "bay-north-coast", name: "Duskendale & Rook's Rest", base: null, sub: true,
    own: [[1,10,"blacks"],[2,4,"greens"]],
    ownBook: [[1,2,"blacks"],[1,4,"greens"]],
    pts: [[1596,1990],[1618,1920],[1660,1880],[1710,1856],[1755,1872],[1735,1935],[1685,1980],[1645,2005]] },

  /* Dragonstone: the black queen's rock. */
  { id: "dragonstone-isle", name: "Dragonstone", base: "iron-throne", nomask: true,
    own: [[1,9,"blacks"]],
    ownBook: [[1,2,"blacks"],[1,7,"greens"],[1,8,"iron-throne"]],
    pts: [[1745,1898],[1786,1906],[1796,1934],[1772,1962],[1738,1954],[1728,1920]] },

  /* Driftmark: the Sea Snake's island, and his fleet with it. */
  { id: "driftmark-isle", name: "Driftmark", base: "iron-throne", nomask: true,
    own: [[1,10,"blacks"]],
    ownBook: [[1,2,"blacks"],[1,8,"iron-throne"]],
    pts: [[1710,2032],[1756,2036],[1766,2062],[1742,2078],[1708,2064]] },

  /* ---------------- the Reach: the Hightowers light the way ---------------- */
  { id: "the-reach", name: "The Reach", base: "iron-throne",
    own: [[1,10,"greens"]],
    ownBook: [[1,2,"greens"],[1,8,"iron-throne"]],
    /* SW border shared cleanly with Dorne (see dorne) — Sun House/Three Towers
       stay Reach, Starfall/Sandstone stay Dorne. */
    pts: [[1080,2060],[1170,2075],[1270,2080],[1330,2140],[1400,2190],[1350,2260],[1330,2380],[1230,2430],[1090,2470],[1010,2560],[940,2620],[925,2760],[912,2880],[905,3010],[560,2970],[430,2500],[480,2260],[640,2270],[740,2240],[850,2200],[950,2140]] },

  /* ---------------- the Stormlands: Borros picks the green letter ---------------- */
  { id: "the-stormlands", name: "The Stormlands", base: "iron-throne",
    own: [[1,10,"greens"]],
    ownBook: [[1,2,"greens"],[1,8,"iron-throne"]],
    pts: [[1400,2190],[1520,2212],[1650,2228],[1950,2230],[1950,2610],[1600,2645],[1430,2625],[1305,2560],[1200,2565],[1100,2620],[1010,2560],[1090,2470],[1230,2430],[1330,2380],[1350,2260]] },

  /* ---------------- Dorne: no part of the Seven Kingdoms, and glad of it ---------------- */
  { id: "dorne", name: "Dorne", base: "martell",
    /* western boundary shares one clean line with the Reach, from the Red
       Mountains (940,2620) to the south coast (905,3010) — no westward overlap. */
    pts: [[1010,2560],[1100,2620],[1200,2565],[1305,2560],[1430,2625],[1600,2645],[1950,2610],[1950,3050],[1300,3050],[905,3010],[912,2880],[925,2760],[940,2620]] },

  /* ---------------- Slaver's Bay: the old cities, untouched by the Dance ---------------- */
  { id: "astapor-lands", name: "Astapor", base: "masters",
    pts: [[3900,2860],[4100,2870],[4130,3000],[4040,3080],[3920,3030]] },
  { id: "yunkai-lands", name: "Yunkai", base: "masters",
    pts: [[3930,2640],[4110,2650],[4140,2760],[4030,2800],[3930,2760]] },
  { id: "meereen-lands", name: "Meereen", base: "masters",
    pts: [[4020,2520],[4200,2530],[4240,2650],[4130,2700],[4040,2660]] },
];

/* The crown itself: every land answering "iron-throne" wears the ruling color.
   Show: Viserys's gold until the green council crowns Aegon (S1E9).
   Book: greens from the coronation, blacks while Rhaenyra holds the city,
   greens again for Aegon's sad half-year, and the boy king's healing gold. */
const CROWN_SHOW = [[1,9,"greens"]];
const CROWN_BOOK = [[1,2,"greens"],[1,5,"blacks"],[1,7,"greens"],[1,8,"iron-throne"]];
function crownDynastyAt(pt) {
  if (!pt) return "iron-throne";
  const tl = pt.type === "book" ? CROWN_BOOK : CROWN_SHOW;
  const a = pt.type === "book" ? pt.b : pt.s;
  const b = pt.type === "book" ? pt.ch : pt.e;
  let d = "iron-throne";
  for (let i = 0; i < tl.length; i++) {
    const seg = tl[i];
    if (seg[0] < a || (seg[0] === a && seg[1] <= b)) d = seg[2];
    else break;
  }
  return d;
}

/* who holds a territory at the given story point (null hides it) */
function territoryRulerAt(t, pt, lore) {
  let ruler = t.base;
  if (!pt) return ruler; // no point chosen: the realm as the tale opens
  const tl = pt.type === "book" ? (t.ownBook || null) : (t.own || null);
  if (!tl) return ruler;
  const a = pt.type === "book" ? pt.b : pt.s;
  const b = pt.type === "book" ? pt.ch : pt.e;
  for (let i = 0; i < tl.length; i++) {
    const seg = tl[i];
    if (seg[0] < a || (seg[0] === a && seg[1] <= b)) ruler = seg[2];
    else break;
  }
  return ruler;
}

/* repaint every territory for the chosen story point & telling */
function applyTerritoryOwners(pt, lore) {
  const layer = document.getElementById("territory-layer");
  if (!layer) return;
  layer.querySelectorAll("[data-territory]").forEach((path) => {
    const t = TERRITORIES.find((x) => x.id === path.getAttribute("data-territory"));
    if (!t) return;
    let ruler = territoryRulerAt(t, pt, lore);
    if (ruler === "iron-throne") ruler = crownDynastyAt(pt); // crown lands wear the ruling color
    if (!ruler) { path.style.display = "none"; return; }
    path.style.display = "";
    const r = RULERS[ruler];
    path.setAttribute("fill", r.color);
    const title = path.querySelector("title");
    if (title) title.textContent = `${t.name} — ${r.name}`;
  });
}

(function () {
  const layer = document.getElementById("territory-layer");
  const pathFor = (t) => {
    const d = "M " + t.pts.map((p) => `${p[0]} ${p[1]}`).join(" L ") + " Z";
    const r = t.base ? RULERS[t.base] : null;
    return `<path class="territory${t.sub ? " territory-sub" : ""}" data-territory="${t.id}" d="${d}"
      fill="${r ? r.color : "#000"}" ${r ? "" : 'style="display:none"'}><title>${t.name}${r ? " — " + r.name : ""}</title></path>`;
  };
  let s = `<defs>
    <mask id="landMask" maskUnits="userSpaceOnUse" x="0" y="0" width="5652" height="3682">
      <image href="../assets/landmask.png" x="0" y="0" width="5652" height="3682" preserveAspectRatio="none"/>
    </mask>
  </defs>
  <g mask="url(#landMask)">`;
  TERRITORIES.filter((t) => !t.nomask).forEach((t) => { s += pathFor(t); });
  s += "</g>";
  // unmasked extras (small isles the landmask may miss)
  TERRITORIES.filter((t) => t.nomask).forEach((t) => { s += pathFor(t); });
  layer.innerHTML = s;
})();
