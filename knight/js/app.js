...
(function () {
  ...
  const state = {
    ...
    lore: localStorage.getItem("kw-knight-lore") || "show", // which telling: "show" | "book"
  ...
  const SEAT_CONTROL = {
    // the black dragon flies over Whitewalls while the Fiddler's wedding-tourney
    // conspiracy lasts (The Mystery Knight) — then Bloodraven's net closes and
    // the castle is forfeit to the crown, to be pulled down stone by stone
    "whitewalls": { start: "butterwell", show: [],
                    book: [[3,5,"blackfyre"],[3,11,"targaryen"]] },
  };
  ...
  function setSite(site) {
    ...
    els.siteTitle.textContent = isWordle ? "WORDLE OF THE REALM" : "A KNIGHT OF THE SEVEN KINGDOMS";
    document.title = isWordle
      ? "Wordle of the Realm - A Knight of the Seven Kingdoms"
      : "A Knight of the Seven Kingdoms Map - Dunk and Egg";
  ...
  const WORDLE_POOL = {
    names: ["AEGON", "AERYS"],
    places: ["DORNE", "NORTH", "REACH", "TWINS"],
    houses: ["ARRYN", "PEAKE", "STARK", "TULLY"],
  ...
  function buildMenus() {
    byId("menu-map").innerHTML = `
      ...
      <label class="legend-row"><input type="checkbox" id="f-banners" ${state.filters.banners ? "checked" : ""}/>
        <span class="legend-banner-icon"><img src="assets/sigils/targaryen.svg" alt=""/></span> House Banners</label>
  ...
  function renderWelcome() {
    els.sidebar.innerHTML = `
      <div class="sidebar-title">A Chronicle of Dunk &amp; Egg</div>
      <p class="sidebar-note">This is the realm as the tale opens, in the two hundred and ninth year after Aegon's Conquest — ninety years before the wolves and lions: the age of good King Daeron, when the black dragon's shadow still lies on the land and a hedge knight rides to a tourney at Ashford Meadow. From here the map follows the whole story, telling by telling.</p>
      <div class="sidebar-divider"></div>
  ...
  function buildSeasonDropdown(expanded) {
    ...
    if (storyShelf === "show") {
      ...
      s += `<div class="dropdown-title">The Tales of Dunk &amp; Egg, Novella by Novella</div>`;
      BOOKS.forEach((book) => {
  ...
  function setLore(lore) {
    state.lore = lore;
    localStorage.setItem("kw-knight-lore", lore);
